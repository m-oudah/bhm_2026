...
@section('title', __('Building Types'))
@section('stylesheet')
    <style>
        .createFormData,
        .editFormData {
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">{{ __('Home') }} /</span> {{ __('Building Types') }}</h4>
        <div class="card">
            <div class="card-datatable pt-0">
                <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
                    <div class="card-header flex-column flex-md-row">
                        <div class="head-label text-center">
                            <h5 class="card-title mb-0">{{ __('Building Types') }}</h5>
                        </div>
    ...
    <div class="createFormData" id="createFormData">
        <div class="offcanvas-header border-bottom mb-3">
            <h5 class="offcanvas-title" id="exampleModalLabel">{{ __('New Building Types') }}</h5>
    ...
    <div class="editFormData" id="editFormData">
        <div class="offcanvas-header border-bottom mb-3">
            <h5 class="offcanvas-title" id="exampleModalLabel">{{ __('Edit Building Types') }}</h5>
            <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"
                id="editClose"></button>
        </div>
        <div class="offcanvas-body flex-grow-1">
            <form id="editData" enctype="multipart/form-data">
                ...
                <div class="col-sm-12">
                    <label class="form-label" for="street_number">{{ __('Name') }}</label>
                    <div class="input-group input-group-merge">
                        <input type="text" id="e_name" class="form-control" name="name" required />
                    </div>
                </div>
                <div class="col-sm-12 mt-3">
                    <button type="submit" class="btn btn-primary data-submit me-sm-3 me-1">{{__('Save')}}</button>
...
@push('scripts')
    ...
    <script>
        ...
        $("#addDataForm").on('submit', function(e) {
            ...
            axios({
                    method: 'post',
                    url: "{{ route('building-types.store') }}",
                    data: formData
                })
        ...
        $('body').on('click', '#deleteRow', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete("{{ url('/') }}" + '/home/building-types/' + id)
                        .then(function(response) {
        ...
        $('body').on('click', '#editRow', function() {
            let id = $(this).data('id');
            let edit = "{{ url('/') }}" + '/home/building-types/' + id + '/edit';
            axios.get(edit)
                .then(function(res) {
                    // console.log(res);
        ...
        $("#editData").on('submit', function(e) {
            ...
            axios({
                    method: 'post',
                    url: "{{ route('building-types.update_data') }}",
                    data: formData
                })
                .then(function(response) {
                    $('#DataTables_Table_0').DataTable().ajax.reload();
