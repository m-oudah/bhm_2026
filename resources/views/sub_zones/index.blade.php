...
@section('content')
        ...
        <div class="offcanvas-body flex-grow-1">
            <form id="addDataForm" enctype="multipart/form-data">
                ...
                <div class="col-sm-12 mt-2">
                    <label for="exampleFormControlSelect1" class="form-label">{{__('Zone')}}</label>
                    <select class="form-select" name="zone_id" aria-label="Default select example">
                      <option selected="">Open this select menu</option>
                      @foreach ($zones as $zone)
                      <option value="{{$zone->id}}">{{$zone->zone_name ? $zone->zone_name : $zone->zone_number }}</option>
                      @endforeach
                    </select>
                  </div>
    ...
    <div class="editFormData" id="editFormData">
        <div class="offcanvas-header border-bottom mb-3">
            <h5 class="offcanvas-title" id="exampleModalLabel">{{ __('Edit Sub Zones') }}</h5>
            <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"
                id="editClose"></button>
        </div>
        <div class="offcanvas-body flex-grow-1">
            <form id="editData" enctype="multipart/form-data">
                ...
                <div class="col-sm-12 mt-2">
                    <label for="exampleFormControlSelect1" class="form-label">{{__('Zone')}}</label>
                    <select class="form-select" id="e_zone_id" name="zone_id" aria-label="Default select example">
                      <option selected="">Open this select menu</option>
                      @foreach ($zones as $zone)
                      <option value="{{$zone->id}}">{{$zone->zone_name ? $zone->zone_name : $zone->zone_number }}</option>
                      @endforeach
...
@push('scripts')
    ...
    <script>
        ...
        $('body').on('click', '#deleteRow', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete("{{ url('/') }}" + '/home/sub-zones/' + id)
                        .then(function(response) {
        ...
        $('body').on('click', '#editRow', function() {
            let id = $(this).data('id');
            let edit = "{{ url('/') }}" + '/home/sub-zones/' + id + '/edit';
            axios.get(edit)
                .then(function(res) {
                    // console.log(res);
                    $('#e_id').val(res.data.id)
                    $('#e_zone_number').val(res.data.zone_number)
                    $('#e_zone_id').val(res.data.zone_id)
                })
        ...
        $("#editData").on('submit', function(e) {
            ...
            axios({
                    method: 'post',
                    url: "{{ route('sub-zones.update_data') }}",
                    data: formData
                })
                .then(function(response) {
                    $('#DataTables_Table_0').DataTable().ajax.reload();
